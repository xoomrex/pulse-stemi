  ...
  def render(assigns) do
    ...
    <div class="user-list" id="dispatch-list">
      <div
        :for={c <- @cases}
        class="user-card"
        style={"cursor: pointer; border-left: 4px solid #{if c.status == "dispatched", do: "#22c55e", else: "#3b82f6"};"}
        phx-click="view_case"
        phx-value-id={c.id}
        id={"dispatch-#{c.id}"}
      >
        <div class="user-card__avatar" style={"background: #{if c.status == "dispatched", do: "#22c55e", else: "#3b82f6"}"} >
        ...
        <div class="user-card__info">
          ...
          <div class="user-card__meta">
            ...
            <span style="color: var(--text-muted); font-size: 12px;">From: {c.phc_user.full_name}</span>
          </div>
        </div>
        <div :if={c.status != "dispatched"} style="color: #3b82f6; font-size: 12px; font-weight: 600;">DISPATCH →</div>
        <div :if={c.status == "dispatched"} style="color: #22c55e; font-size: 12px; font-weight: 600;">✓ DONE</div>
      </div>
